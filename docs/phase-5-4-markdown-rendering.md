...
- Glamour gem is in Gemfile but has Go FFI issues (same as Lipgloss)
...
**Implementation:**
```ruby
# lib/prompt_objects/ui/markdown_renderer.rb
...
### Option B: Wait for Glamour FFI Fix

Continue using raw markdown and wait for Marco to fix the Glamour Go FFI issues.
...
```ruby
def self.h1
  style(bold: true, foreground: "#FFFFFF")
end

def self.h2
  style(bold: true, foreground: "#AAAAFF")
end

def self.h3
  style(bold: true, foreground: "#88AAFF")
end
...
def self.code_span
  style(foreground: "#FF8888", background: "#333333")
end

def self.code_block
  style(foreground: "#AAFFAA", background: "#1A1A1A")
end
```

### Step 3: Integrate with Conversation

Update `conversation.rb` to use MarkdownRenderer for assistant messages:
```ruby

  ...
  if (entries.length === 0) {
    return (
      <div className="h-full flex items-center justify-center">
        <span className="font-mono text-2xs text-po-text-ghost">No shared data</span>
      </div>
    )
  }
...
function EnvDataRow({ entry, expanded, onToggle }: { entry: EnvDataEntry; expanded: boolean; onToggle: () => void }) {
  return (
    <div className="border-b border-po-border last:border-b-0">
      <button
        onClick={onToggle}
        className="w-full text-left px-1.5 py-1 hover:bg-po-surface-3 transition-colors duration-150 flex items-center gap-2"
      >
        <span className="text-2xs text-po-text-ghost">{expanded ? '▼' : '▶'}</span>
        <span className="font-mono text-xs text-po-accent truncate">{entry.key}</span>
        <span className="text-2xs text-po-text-ghost truncate flex-1">{entry.short_description}</span>
        <span className="text-2xs text-po-text-ghost flex-shrink-0">{entry.stored_by}</span>
      </button>
      {expanded && (
        <div className="px-2 pb-2">
          <div className="text-2xs text-po-text-ghost mb-1">
          ...
          <pre className="font-mono text-2xs text-po-text-primary bg-po-surface-1 rounded p-2 overflow-auto max-h-40 whitespace-pre-wrap break-all">
